...
| `rate.py` | `RateCell`, the population rate cell the `레이트` tab drives — N near-barrier PAIRS latches hold a past symbol's analog DEPTH as a COUNT (where one latch keeps only the sign), then a fixed current symbol is MULTIPLIED by a positive gain from that count so it reads differently by the held past (context) while keeping its own sign. Drives `CoupledEngine`, adds no physics. `chain=` (default `CHAIN`=0.0) = disintegrated (holds depth, not Phi) — where every published number was taken (`default-stays-exact`, pinned by `tests/test_rate_chain.py`); but a CHOICE, since `INTEGRATED_CHAIN`=0.05 measures INTEGRATED at 6 units (phi_proxy decay test, not the exact-Phi magnitude — a width-artefact) at no measured depth cost (scoped to 6 units; the proxy floor is untrustworthy at 32 pairs). Measured by `state/communication/{rate_code,context_modulation,context_synergy,integrated_rate,rate_chain}.py` | (new — not in the origin) |
