...
- `DialogueSession` 완주 212 시행 0.36초. 멱등 제출(한 시행 = 한 로그 행) · 무입력 step 은 no-op ·
  완주 로그는 한 번만 넘어감 — 모두 테스트로 고정(`tests/test_dialogue_session.py`, 10건).
...
- sol 이 더 센 대조군을 제안했다(두 맥락 동시 live/yoked · 매-시행 디스플레이-정체성 뒤섞기 귀무 ·
  ...
  행동 검사로 쓰는 안을 택했고(더 단순하고 기존 뷰어 구조에 맞음), **sol 의 강화안은 후속으로 남긴다.**
