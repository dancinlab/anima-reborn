...
**다음(진행 중)**: 이 프로토타입을 `src/sequence.py` 엔진으로 승격(step/reset · lockstep 뷰어 탭 —
테이프가 차고 유지되는 것을 보여주는 수동 시각화) + 스크립트를 출하 엔진 구동으로 재배선. 그다음
부품 2(`phi_proxy.py` — 측정가능성 프록시), 부품 4(조건부 합성 — 작업기억 대화).
