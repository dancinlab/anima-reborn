...
<p align="center">
  ...
  <img alt="Tests" src="https://img.shields.io/badge/tests-238%20passing-success">
...
일어납니다. 단, **그냥 해 보면 거짓말을 하는 실험**이라 두 가지를 먼저 처리해야 했습니다.
둘 다 코드를 쓰기 전에 측정했습니다.
...
모든 측정에 **귀무대조(null)** 가 따라옵니다 — 같은 단어들을 무작위로 짝지어 똑같이 잰 값.
...
| --- | ---: | ---: | ---: | --- |
| 같은 단어열 | 0.427 | 0.009 | **+0.418** | 창발 ✨ |
| 절반만 공유 | 0.117 | 0.008 | +0.109 | 부분적 |
| 한 칸 밀림 | 0.098 | 0.008 | +0.090 | 부분적 |
| 완전히 무관 | 0.009 | 0.009 | **+0.001** | 독립 |

귀무값이 관계와 **무관하게 0.008~0.009로 평평한** 것이 핵심입니다 — 그건 단어가 아니라
...
결정하기 때문입니다. 규칙이 결정할 수 **없는** 것은 같은 규칙을 통과한 귀무값 위의 초과분이고,
