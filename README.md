...
대조군 여백은 **12/12** 양수입니다. 자기 순열 상한은 **11/12** — 한 시드가 못 넘었고,
입력 수준에서 가장 낮았던 그 시드입니다. 그래서 "깨끗하게 성립"이라고 쓰지 않고, 못 넘은
시드를 지우지도 시드 수를 바꾸지도 않습니다.
...
#### ⚠ 정확도만 떼어 읽으면 안 되는 이유 — 유효 랭크 1.35

학습된 표상은 4차원 중 **약 1.35차원**만 씁니다. 원신호(1.93)보다 오히려 **낮습니다** —
중간점 규칙이 둘을 끌어모으니 공간이 줄어드는 게 당연합니다. 붕괴점(랭크 1 · 정보 0)은
아니지만 거의 직선이고, 벡터 입력 용량의 대부분이 놀고 있습니다. D7 이 남긴 교훈("일치율은
쓰인 유역 수와 함께 보지 않으면 의미가 없다")을 이 실험에 적용한 열입니다.
